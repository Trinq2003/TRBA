...
def initialize(args):
    ...
    trainset = HWDataset(root_dir, min_size=img_width, max_size=img_height, mode='train', split_type=args.split_type, max_len=args.max_length)
    testset = HWDataset(root_dir, min_size=img_width, max_size=img_height, mode='test', split_type=args.split_type, max_len=args.max_length)
    ...
    print(f"[INFO] Trainset size: {len(trainset)}")
    print(f"[INFO] Testset size: {len(testset)}")
    print("="*65)

    # print("="*25, f"Model Structure", "="*25)
    # summary(model)
    # print("="*65)
